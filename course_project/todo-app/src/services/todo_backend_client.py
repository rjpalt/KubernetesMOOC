...
class TodoBackendClient:
    ...
    async def get_all_todos(self) -> List[Todo]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503, 
                detail="Todo backend service unavailable"
            )
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error when fetching todos: {e}")
            raise HTTPException(
                status_code=e.response.status_code,
                detail="Error fetching todos from backend"
            )
    ...
    async def create_todo(self, text: str) -> Todo:
        ...
        try:
            ...
            raise HTTPException(
                status_code=e.response.status_code,
                detail="Error creating todo in backend"
            )
    ...
    async def update_todo(self, todo_id: str, text: str = None, status: TodoStatus = None) -> Todo:
        ...
        try:
            ...
            raise HTTPException(
                status_code=e.response.status_code,
                detail="Error updating todo in backend"
            )
    ...
    async def delete_todo(self, todo_id: str) -> bool:
        ...
        try:
            ...
            raise HTTPException(
                status_code=e.response.status_code,
                detail="Error deleting todo in backend"
            )
