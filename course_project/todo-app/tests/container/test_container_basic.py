...
def test_docker_build_succeeds():
    ...
    assert (
        "Successfully tagged todo-app:test" in output
        or "Successfully built" in output
        or "naming to docker.io/library/todo-app:test" in output
    ), f"Build success not detected in output: {output[:500]}"
...
def test_container_environment_setup():
    ...
    result = subprocess.run(
        ["docker", "run", "--rm", "todo-app:test", "python", "--version"], capture_output=True, text=True, timeout=30
    )
