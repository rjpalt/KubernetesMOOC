...
class TodoDatabase:
        ...
        async with session as s:
            try:
                result = await s.execute(
                    select(TodoDB).where(TodoDB.id == todo_id_int)
                )
    ...
    async def get_all_todos(self) -> List[Todo]:
        ...
        async with session as s:
            try:
                result = await s.execute(
                    select(TodoDB).order_by(TodoDB.created_at.desc())
                )
    ...
    async def update_todo(self, todo_id: str, text: Optional[str] = None, 
                         status: Optional[TodoStatus] = None) -> Optional[Todo]:
        """Update a todo item."""
        try:
            todo_id_int = int(todo_id)
        except ValueError:
    ...
    async def delete_todo(self, todo_id: str) -> bool:
        ...
        async with session as s:
            try:
                result = await s.execute(
                    delete(TodoDB).where(TodoDB.id == todo_id_int)
                )
                await s.commit()
