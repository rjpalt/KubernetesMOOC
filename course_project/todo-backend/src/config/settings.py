...
class Settings(BaseSettings):
    ...
    # NATS configuration (optional for graceful degradation)
    nats_url: str = Field(default="", description="NATS server URL (empty = auto-detect)")
    nats_topic: str = Field(default="todos.events", description="NATS topic for todo events")
    nats_enabled: bool = Field(default=True, description="Enable NATS message publishing")
    ...
    def effective_nats_url(self) -> str:
        """Get environment-appropriate NATS URL with auto-detection."""
        if self.nats_url:
