...
class TestTodoUpdate:
    ...
    def test_valid_todo_update_both_fields(self):
        ...
        update_data = TodoUpdate(text="Updated text", status=TodoStatus.DONE)
...
class TestTodo:
    ...
    def test_todo_default_status(self):
        ...
        todo = Todo(id="test-id", text="Test todo", created_at=datetime.now())
    ...
    def test_todo_serialization_for_api(self):
        ...
        todo = Todo(
            id="test-id", text="Test todo", status=TodoStatus.NOT_DONE, created_at=datetime(2025, 7, 21, 10, 0, 0)
        )
    ...
    def test_todo_text_validation_in_creation(self):
        ...
        # Empty text should fail
        with pytest.raises(ValidationError):
            Todo(id="test-id", text="", created_at=datetime.now())

        # Too long text should fail
        with pytest.raises(ValidationError):
            Todo(id="test-id", text="a" * 141, created_at=datetime.now())
