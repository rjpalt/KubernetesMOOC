...
- [4.4](https://github.com/rjpalt/KubernetesMOOC/tree/4.4/ping-pong) - **Argo Rollouts with Azure Managed Prometheus Integration and a Sidecar Authentication Proxy**
  - The implementation is a bit different from course exercises as I use an Azure managed Prometheus solution instead of a self-managed Prometheus server.
  - The implementation requires checking from Azure side the prometheus metrics, which, in turn, requires authentication. Due to to reasons I am unable to make an Entra ID app to handle the OAuth2 authentication and the solution implementes authentication in a different way.
  - The solution uses a sidecar authentication proxy done with Flask. The sidecar authenticates to Azure Manged Prometheus using a managed identity and provides Argo Rollouts a valid token to access the metrics. The sidecar source code is located in a [separate folder](/auth-proxy-sidecar/)
...
- [4.8](https://github.com/rjpalt/KubernetesMOOC/blob/4.8/cluster-manifests/main-app.yaml) - GitOps Migration
  ...
  - [Image of ArgoCD deployment pushed via GitHub Actions](https://github.com/user-attachments/assets/33d24afe-5389-4743-972d-61efea6b2748)
